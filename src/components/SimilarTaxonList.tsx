...
function SimilarTaxonList({ similar }: { similar: SimilarTaxon[] }) {
    ...
    const { isPending, error, data, isFetching } = useTaxons(undefined, (item) => similar.find((x) => x.latin === item.latin) !== undefined)
    ...
    return (
        <SimilarWrapper>
            ...
            <SimilarContent open={open}>
                {data.map((item: Taxon) => {
                    return <TaxonGroupedPhoto taxon={item} key={item.artId} stage={'imago'}></TaxonGroupedPhoto>
                })}
            </SimilarContent>
